...
const state = {
  ...
  // fly mode (toggled by backtick `): when true gravity is disabled and you can ascend/descend
  flyMode: false,
  flySpeed: 6,        // vertical speed while flying (m/s)
...
window.addEventListener('keydown', (e) => {
  ...
  // Flight binding toggles fly mode (respects remappable key in Controls)
  // Determine current flight key from bindings (fallback to backtick if not set)
  const flightKey = (state.controls && state.controls.flight) || '`';
  if (e.key === flightKey) {
    state.flyMode = !state.flyMode;
    // when enabling fly, zero vertical physics so player doesn't fall
    if (state.flyMode) {
      state.vy = 0;
      state.grounded = false;
      // show a brief achievement-like notification (non-critical)
      try { showAchievement('Flight mode', state.flyMode ? 'Enabled' : 'Disabled'); } catch (e) {}
    } else {
      // leaving fly: keep current height but ensure vy reset so gravity resumes cleanly
      state.vy = 0;
    }
  }
  ...
  if (e.code === 'Space') {
    // If flying, treat Space as ascend input only (no double jump logic)
    if (state.flyMode) {
      // nothing to mutate here except prevent page scroll
      e.preventDefault();
    } else {
...
(function setupCustomTab() {
    ...
    customPanel.innerHTML = `
      ...
      <label style="font-size:13px; color:var(--ui); display:block; margin-bottom:8px;">
        ...
        <div style="display:flex; gap:8px; align-items:center; margin-top:6px;">
          <input id="custom-model-file" type="file" accept=".gltf,.glb,.fbx,.obj,.mtl,.blend" multiple style="flex:1; pointer-events:auto;">
          <button id="custom-unload" type="button" style="pointer-events:auto; padding:8px 10px; border-radius:6px; border:0; background:rgba(255,255,255,0.06); color:var(--ui);">Unload</button>
        </div>
      </label>
      ...
      <!-- Scale control (new): allows sizing the model before placement -->
      <div style="display:flex; gap:8px; align-items:center; margin-bottom:8px;">
        <label style="font-size:13px; color:var(--ui);">Scale</label>
        <input id="custom-model-scale" type="range" min="0.1" max="4" step="0.05" value="1" style="flex:1; pointer-events:auto;">
        <span id="custom-model-scale-val" style="width:44px; text-align:right; font-weight:700; color:var(--ui);">1.00</span>
      </div>
      ...
      <!-- Model preview (shows scaled preview) -->
  ...
  const unloadBtn = document.getElementById('custom-unload');
  ...
  // render preview if model present (now respects user scale control)
  // NOTE: don't clear the entire previewScene (that would remove lights). Instead remove only the previous preview model.
  let __previewModel = null;
  function renderPreview() {
    if (!loadedModel || !previewRenderer || !previewScene || !previewCamera) return;
    try {
      // remove previous model but preserve lights and other helpers
      if (__previewModel && __previewModel.parent === previewScene) {
        previewScene.remove(__previewModel);
        // dispose geometries/textures of previous preview model conservatively
        __previewModel.traverse((m) => {
          if (m.isMesh) {
      ...
      // clone the loaded model for preview so we don't mutate the original
      const clone = loadedModel.clone(true);

      // compute bounding box after adding to scene so lights/shaders behave consistently
      // temporarily add to scene to let Box3 compute proper values
      ...
      // user-controlled scale value (from slider)
      const userScaleEl = document.getElementById('custom-model-scale');
      const userScale = userScaleEl ? (parseFloat(userScaleEl.value) || 1) : 1;
      const finalScale = fitScale * userScale;

      // apply final scale and re-center
      ...
      // ensure clone is the only model in the scene besides lights
      __previewModel = clone;

    } catch (e) {
      // on error, ensure there's no dangling preview model
      try { if (__previewModel && __previewModel.parent === previewScene) previewScene.remove(__previewModel); } catch (ee) {}
      __previewModel = null;
    }

    // finally render the preview scene
    try {
  ...
  // place model into world at pos (Vector3) with optional user scale
  function placeModelAt(pos, scale = 1) {
    ...
    // apply user scale immediately so bounding box reflects final size
    try {
    ...
    if (!box.isEmpty()) {
      // box.min.y is the lowest point in local/world after applying placed.position and scale
    ...
    const proxyBox = (function() {
      // recompute bbox after placement adjustments (and after scale applied)
  ...
  // wire scale control UI so preview and apply use it
  const scaleInputEl = document.getElementById('custom-model-scale');
  const scaleValEl = document.getElementById('custom-model-scale-val');
  if (scaleInputEl) {
    // reflect initial value
    if (scaleValEl) scaleValEl.textContent = parseFloat(scaleInputEl.value || 1).toFixed(2);
    scaleInputEl.addEventListener('input', (ev) => {
      if (scaleValEl) scaleValEl.textContent = parseFloat(ev.target.value || 1).toFixed(2);
      // update preview live
  ...
  // Unload button: clears only the currently staged custom model & preview (keeps placed models intact)
  if (unloadBtn) {
    unloadBtn.addEventListener('click', (ev) => {
      ev.preventDefault();
      try {
        // reuse refreshCustomPanel() helper to clear staged model resources and preview UI
        if (typeof refreshCustomPanel === 'function') {
          refreshCustomPanel();
        } else {
          // fallback: replicate a minimal unload if helper not available
          if (loadedUrl) { try { URL.revokeObjectURL(loadedUrl); } catch (e) {} loadedUrl = null; }
          loadedModel = null;
          placementPos = null;
          placementValid = false;
          placementChosen = false;
          ...
          if (previewEl) previewEl.innerHTML = 'No model loaded';
        }
      } catch (e) {}
    });
  }
  ...
  // helper: clear any staged custom model and preview renderer (do not touch placed models in world)
  function refreshCustomPanel() {
    try {
      ...
      // revoke any temporary URL used for loading
      try { if (loadedUrl) { URL.revokeObjectURL(loadedUrl); loadedUrl = null; } } catch (e) {}

      // dispose preview renderer and remove its canvas if present
      try {
        if (previewRenderer) {
          try { previewRenderer.dispose(); } catch (e) {}
          const pcanvas = previewRenderer.domElement;
  ...
  document.querySelectorAll('.subtab-btn').forEach(b => {
    b.addEventListener('click', () => {
      ...
      if (sub === 'custom') {
        ...
        // clear any staged/temporary model state so reopening Custom starts fresh
        try { refreshCustomPanel(); } catch (e) {}
      } else {
...
function animate() {
  ...
  // Vertical physics (gravity + jump) OR Fly mode vertical control
  let landedOnObject = false;
  if (state.flyMode) {
    // In fly mode: ignore gravity and use jump/crouch bindings for vertical movement
    const up = bindingPressed('jump') || false; // Space or remapped jump
    const down = bindingPressed('crouch') || keys['ctrl'] || false; // Ctrl or remapped crouch
    const vs = state.flySpeed * (keys['shift'] ? state.sprintMultiplier : 1);
    // ascend
    if (up && !down) {
      yaw.position.y += vs * dt;
    } else if (down && !up) {
      yaw.position.y -= vs * dt;
      // keep above minimum floor height
      if (yaw.position.y < state.height * 0.5) yaw.position.y = state.height * 0.5;
    }
    // ensure vy remains zero and grounded false while flying
    state.vy = 0;
    state.grounded = false;
  } else {
    // integrate velocity
